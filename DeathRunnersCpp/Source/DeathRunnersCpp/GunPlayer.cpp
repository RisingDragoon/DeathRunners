...
void AGunPlayer::SpecialAbility()
{
	...
	if (SpecialAbilityIsReady)
	{
		if (LastAxisY >= 0)
		{
			...
			//FVector location = SpawnPosition;
			//FVector StartTrace = SpawnPosition;
			SpawnPosition = GetActorLocation() + SpawnPositionOffset;
			...
			AProjectile* proj = GetWorld()->SpawnActor<AProjectile>(Projectile, SpawnPosition, FRotator());//SpawnPosition
